...
function getBlogDetails(blog_id) {
    ...
    blog.then((response) => {
        if (response['data']['postDetails']) {
            detailsNewas.value = {};
            detailsNewas.value = response['data']['postDetails'] ? response['data']['postDetails'] : '';
            console.log(detailsNewas.value)
            laoder.value = false
        }else{
            alert("Api has no data for this blog id");
            laoder.value = false
        }
    })
}
...
<template>
    <section class="py-6 sm:py-12">
        <div class="container p-6 mx-auto space-y-8">
            <div v-show="detailsNewas == ''">
                <div class="space-y-2 text-center">
                    <h2 class="text-3xl font-bold mb-4">সাম্প্রতিক খবর</h2>
                </div>
            ...
            <BlogDetails v-show="detailsNewas != ''" :detailsNewas="detailsNewas" />

            <div v-show="laoder" class="flex items-center justify-center ">
